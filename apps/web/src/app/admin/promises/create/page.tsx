...
export default function CreatePromisePage() {
  const [newPromise, setNewPromise] = useState({ title: "", description: "", icon: "", order: "0", is_active: true });
    ...
    <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm border border-slate-100 dark:border-slate-800">
        <form id="promise-form" onSubmit={handleCreate} className="space-y-6">
          ...
          <Input label="Title" value={newPromise.title} onChange={e => setNewPromise({...newPromise, title: e.target.value})} required className="bg-slate-50/50" />
